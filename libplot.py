...
def plot_poison_contours(ax, attack, xx, yy, **params):
    Z = np.vectorize(attack.objective_function, signature='(2)->()')(
        np.concatenate(
            (xx.reshape((xx.size, 1)),
            yy.reshape((yy.size, 1))), axis=1
        )
    )
    if type(Z) is not np.ndarray:
        Z = Z.tondarray()
...
def plot_poison(clf, X_labelled, y_labelled, X_unlabelled, y_unlabelled, X_test, y_test, attack, attack_points, start_points, start_points_y, ax=None):
    """
    ...
    for start, end, start_label in zip(start_points, attack_points, start_points_y):
        ...
        ax.arrow(start[0], start[1], end[0]-start[0], end[1]-start[1], 
         ...
         zorder=1, head_length=0.15, head_width=0.1)
        
        
def c_plot_poison(X_labelled, y_labelled, attack, lb, ub, attack_points, start_points, start_points_y, x_seq, ax=None):
    if ax is None:
        fig, ax = plt.subplots(1,1, figsize=(15,15))
    ...
    if x_seq is not None:
        ...
        path = [
            (Path.MOVETO, x_seq[0]),
            *[
                (Path.LINETO, x) for x in x_seq[1:-1]
            ]
        ]
        ...
        #ax.add_patch(arrow)
        if len(x_seq) >= 2:
            ax.arrow(x_seq[-2][0], x_seq[-2][1], x_seq[-1][0]-x_seq[-2][0], x_seq[-1][1]-x_seq[-2][1], 
                shape='full', color='black', length_includes_head=True, 
                 zorder=10, head_length=0.05, head_width=0.05
            )
