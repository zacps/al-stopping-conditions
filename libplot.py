...
def plot_paraeto_hull(
    ...
    for ds in range(len(datasets)):
        ...
        ax[r, c].plot(pareto_front[:, 0], pareto_front[:, 1], c='#777777', ls='--', zorder=1, 
                      label='Pareto frontier' if ds == 0 else None) # Kat
        ...
        ax[r,c].set_position([box.x0, box.y0 + box.height * 0.1,
                         box.width, box.height * 0.9])
        
        # grey area under pareto front
        py = pareto_front[:, 1][np.logical_not(np.isnan(pareto_front[:, 1]))] # Kat
        ax[r,c].fill_between(
            np.append(pareto_front[:, 0][np.logical_not(np.isnan(pareto_front[:, 0]))], [ax[r,c].get_xlim()[1]]), 
            ax[r,c].get_ylim()[0], 
            np.append(py, [py[-1]]),
    ...
    plt.legend(ls, [l.get_label() for l in ls], bbox_to_anchor=legend_bbox, loc=legend_loc, ncol=legend_cols, frameon=False)
    return fig, ax