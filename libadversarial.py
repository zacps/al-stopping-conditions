...
def fgm(
    ...
    **kwargs
    ...
    result = (idx[:n_instances], adversarial_examples[idx[:n_instances]] if teach_adversarial else None)
...
def adversarial(
    ...
    **attack_kwargs
    ...
    if parallel_threads != 1:
        ...
        adversarial_examples = Parallel(n_jobs=parallel_threads)(delayed(attack.generate)(part) for part in np.array_split(x, parallel_threads))
        adversarial_examples = np.array(adversarial_examples).reshape(-1, 2)
    else:
        adversarial_examples = attack.generate(X)
...
def comparative_density(
    ...
    **attack_kwargs
    ...
    density = np.sum(euclidean_distances(X_unlabelled, X_labelled), axis=1) - np.sum(euclidean_distances(X_unlabelled, X_unlabelled), axis=1)
...
def poison_secml(
    ...
    **kwargs,
    ...
    # Select start points for the poisoning attack
    start_points_idx, start_points = point_selector(classifier, X_train, n_instances=n_instances)
    ...
    _labels, _score, points, _final_objective = attack.run(start_points, CArray(start_points_labels))
    points = points.X.tondarray()
        
    return (None, points, start_points, attack, attack.x_seq)
...
def random_synthesis(
    ...
    **kwargs
    ...
    return (None, points.T, None, None, None)
