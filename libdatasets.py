...
def hiv(dataset_size=1000):
    ...
    dataset = pd.read_csv(
        r"F:\Downloads\compound_datasets\collection\HIV\dataset_cddd.csv"
    )
...
def mutagen(dataset_size=1000):
    ...
    dataset = pd.read_csv(
        r"F:\Downloads\compound_datasets\collection\Mutagenicity\dataset_cddd.csv"
    )
...
def MUV(dataset_id, dataset_size=1000):
    ...
    dataset = pd.read_csv(
        r"F:\Downloads\compound_datasets\collection\MUV\dataset_cddd.csv"
    )
    dataset = dataset[dataset[str(dataset_id)] != "?"]
    y = dataset[str(dataset_id)].to_numpy("int")
    X = dataset.iloc[
        :, dataset.columns.get_loc("cddd_1") : dataset.columns.get_loc("cddd_512")
    ].to_numpy("float")
...
def sider(dataset_size=1000):
    ...
    dataset = pd.read_csv(
        r"F:\Downloads\compound_datasets\collection\SIDER\dataset_cddd.csv"
    )
...
def tox21(dataset_id, dataset_size=1000):
    ...
    dataset = pd.read_csv(
        r"F:\Downloads\compound_datasets\collection\tox21\dataset_cddd.csv"
    )
    dataset = dataset[dataset[str(dataset_id)] != "?"]
    y = dataset[str(dataset_id)].to_numpy("int")
    X = dataset.iloc[
...
def newsgroups(dataset_size=1000, categories=None):
    bunch = datasets.fetch_20newsgroups_vectorized(
        subset="all", remove=("headers"), normalize=True
    )
    X = bunch.data
    y = bunch.target
...
def newsgroups_svd(dataset_size=1000, categories=None):
    ...
    X = sklearn.decomposition.TruncatedSVD(
...
def higgs(dataset_size=1000):
    ...
    if cache is not None:
        ...
        return X, y
    dataset = pd.read_csv(
        r"https://archive.ics.uci.edu/ml/machine-learning-databases/00280/HIGGS.csv.gz",
        compression="gz",
    )
    y = dataset[0].to_numpy()
...
def spamassassin(dataset_size=1000):
    ...
    if cache is not None:
        ...
        return X, y
    spam = [
        "datasets/spamassasin/20030228_spam.tar.bz2",
        "datasets/spamassasin/20050311_spam_2.tar.bz2",
    ]
    ham = [
        ...
        with tarfile.open(filename, "r:bz2") as archive:
            for file in archive:
                if not file.isfile():
                    continue
                docs.append(
                    (archive.extractfile(file).read().decode("latin-1"), "spam")
                )
    for filename in ham:
        with tarfile.open(filename, "r:bz2") as archive:
            for file in archive:
...
def malware(dataset_size=1000):
    ...
    if cache is not None:
        ...
        return X, y
    cols = (
        set(pd.read_csv("datasets/malware/staDynVt2955Lab.csv").columns)
        & set(pd.read_csv("datasets/malware/staDynVxHeaven2698Lab.csv").columns)
        & set(pd.read_csv("datasets/malware/staDynBenignLab.csv").columns)
    )

    files = [
        "datasets/malware/staDynVt2955Lab.csv",
        "datasets/malware/staDynBenignLab.csv",
        "datasets/malware/staDynVxHeaven2698Lab.csv",
    ]
    ...
    if dataset_size is not None:
        X, _, y, _ = train_test_split(
            X.to_numpy(), y.to_numpy(), train_size=dataset_size, random_state=42
        )
...
def coral(dataset_size=1000):
    ...
    if cache is not None:
        ...
        return X, y
    r = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/00298/jsbach_chorals_harmony.zip",
        stream=True,
    )
...
def buzz(site, dataset_size=1000):
    ...
    if not exists(path):
        r = requests.get(
            "http://ama.liglab.fr/data/buzz/classification.tar.gz", stream=True
        )
    ...
    with tarfile.open(path, mode="r:") as z:
        ...
        data_tw = pd.read_csv(
            z.extractfile(
                "classification/Twitter/Absolute_labeling/Twitter-Absolute-Sigma-500.data"
            ),
            header=None,
        )
...
def dota2(dataset_size=1000):
    ...
    r = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/00367/dota2Dataset.zip",
        stream=True,
    )
...
def gas(dataset_size=1000):
    ...
    with zipfile.ZipFile(path) as z:
        datas = [
            datasets.load_svmlight_file(z.open(f"Dataset/batch{i}.dat"))
            for i in range(1, 11)
        ]
...
def _normalize(X, y):
    X = StandardScaler(
        with_mean=not isinstance(X, scipy.sparse.csr_matrix)
    ).fit_transform(X)
    return X, y
